...
def parse_line(line):
    ...
    if sgpr_usage == 104:
        print(filename,bb_addr,sgpr_usage,vgpr_usage)
...
def plot_data(data,goal,filename,start_addr, max_gpr_claimed):
    ...
    """
    if "v" in goal:
        ax.set_title("min_%s_avail = %u\n"%(goal,min_data))
    else:
        ax.set_title("min_%s_avail = %u\n"%(goal,min_data ))
    ytop = 30000 if goal == "sgpr" else 20000
...
def plot_2d_data(sgpr_list,vgpr_list,filename,start_addr, max_sgpr_claimed , max_vgpr_claimed):
    ...
    ax.set_title("min_sgpr_avail = %u, min_vgpr_avail = %u\n"%(min_sgpr,min_vgpr))
    #ax.set_title("file: %s\nkern_addr = 0x%#x, min_sgpr_avail = %u, min_vgpr_avail = %u\n, sgpr claimable = %u, vgpr claimable = %u"%(filename,start_addr,min_sgpr,min_vgpr,max(102-max_sgpr_claimed,0), 256 - max_vgpr_claimed))
...
# TOTAL 2D distribution
print("no sgpr")
for sk in empty_sgpr_kernels:
    print(sk)
print("no vgpr")
for vk in empty_vgpr_kernels:
    print(vk)
print("no both")
for svk in empty_vgpr_kernels.intersection(empty_sgpr_kernels):
    print(vk)


for avail in range(1,10):
    sgpr_constrained = (len(list(filter(lambda sgpr_avail : sgpr_avail >= avail, total_sgpr_list))) )
    print(" %d or more sgpr available percentage = %.6f\n"%(avail,sgpr_constrained * 1.0 / len(total_sgpr_list)))

for avail in range(1,10):
    vgpr_constrained = (len(list(filter(lambda vgpr_avail : vgpr_avail >= avail, total_vgpr_list))) )
    print(" %d or more vgpr available percentage = %.6f\n"%(avail,vgpr_constrained * 1.0 / len(total_vgpr_list)))

print("MAX SGPR CLAIMED",max_sgpr_claimed)
